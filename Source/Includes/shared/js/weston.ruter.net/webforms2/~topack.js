...
hasBadImplementation: navigator.userAgent.indexOf('Chrome'),
...
if(!_2)
_2 = document.getElementsByTagName('*')[0];
...
_23 &&
_23.documentElement &&
/:?\bselect$/i.test(_23.documentElement.nodeName) &&
_23.documentElement.namespaceURI == 'http://www.w3.org/1999/xhtml'
)
{
...
_23 &&
_23.documentElement &&
/:?\bformdata$/.test(_23.documentElement.nodeName) &&
_23.documentElement.namespaceURI == 'http://n.whatwg.org/formdata'
)
{
var rt;
var root = _23.documentElement;
...
$wf2._65.apply(repetitionBlocks[i]);
},
...
if(!this.firstChild)
this.appendChild(document.createTextNode($wf2.repetitionButtonDefaultLabels[_7]));
if(_7 != 'add')
this.disabled = !$wf2._4d(this);
else {
var rb;		
this.disabled = !(((rb = $wf2._4d(this)) && rb.repetitionTemplate)
||
this.htmlTemplate
);
}
if(this.addEventListener)
this.addEventListener('click', $wf2._66, false);
else if(this.attachEvent)
this.attachEvent('onclick', $wf2._66);
else this.onclick = $wf2._66;
this.wf2Initialized = true;
},
initRepetitionButtons : function(_7, _2){
var i;
if(!_2)
_2 = document.documentElement;
var inpts = $wf2._51.apply(_2, [['input'], 'type', _7]);
for(i = 0; i < inpts.length; i++){
var btn = document.createElement('button');
for(var j = 0, _4; _4 = inpts[i].attributes[j]; j++)
btn.setAttribute(_4.nodeName, inpts[i].getAttribute(_4.nodeName));
inpts[i].parentNode.replaceChild(btn, inpts[i]);
btn = null;
}
var _40 = $wf2._51.apply(_2, [['button'], 'type', _7]);
for(var i = 0; i < _40.length; i++)
$wf2._67.apply(_40[i], [_7]);
},
_66 : function(e){
...
if(!_c){
_c = this;
while(_c.previousSibling && _c.previousSibling.repetitionType != 2)
...
this.repetitionBlocks.push(block);
if($wf2._69)
this.repetitionBlocks.sort($wf2._69);
}
...
try {
if(document.createEvent)
_d = document.createEvent('UIEvents');
else if(document.createEventObject)
_d = document.createEventObject();
...
this.repetitionTemplate.dispatchEvent(_d);
...
var _2d;
if((_2d = this.repetitionTemplate.getAttribute('onremoved')) &&
...
else if((_2d = this.repetitionTemplate.getAttribute('onremove')) &&
...
if(_3d[i].htmlTemplate == this.repetitionTemplate)
_3d[i].disabled = false;
}
}
}
},
...
if(_5 < 0){
while(_5 != 0 && target.previousSibling && 
target.previousSibling.repetitionType != 1)
{
target = target.previousSibling;
if(target.repetitionType == 2)
_5++;
}
}
else {
...
try {
if(document.createEvent)
_a = document.createEvent('UIEvents');
else if(document.createEventObject)
_a = document.createEventObject();
RepetitionEvent._upgradeEvent.apply(_a);
_a.initRepetitionEvent('moved', true, false, this);
if(this.repetitionTemplate.dispatchEvent)
this.repetitionTemplate.dispatchEvent(_a);
else if(this.repetitionTemplate.fireEvent){
}
}
catch(err){
...
var _2d;
if((_2d = this.repetitionTemplate.getAttribute('onmoved')) &&
...
var node;
if(_4 && (node = document.getElementById(_4)) && node.getAttribute('repeat') == 'template' )
return node;
return null;
},
_72 : function(rt){
...
frm.addEventListener('submit', $wf2._64, false);
else
frm.attachEvent('onsubmit', $wf2._64);
}
...
if(_2.getAttribute('autofocus'))
els.unshift(_2);
for(i = 0; i < els.length; i++)
$wf2._5b(els[i]);
var textareas = $wf2._51.apply(_2, [['textarea'], 'maxlength']);
if(_2.nodeName.toLowerCase() == 'textarea')
textareas.unshift(_2);
...
if($wf2._53(node, 'visibility') == 'hidden' || $wf2._53(node, 'display') == 'none')
return;
...
jslog.debug('setting focus on ')
el.focus();
setTimeout(
function() {
EventHelpers.fireEvent(el, 'focus');
}
, 10)
...
try {
if(document.createEvent)
_6 = document.createEvent('Events');
else if(document.createEventObject)
_6 = document.createEventObject();
_6.initEvent('invalid', true, true );
_6.srcElement = this;
if(this.dispatchEvent)
canceled = !this.dispatchEvent(_6);
else if(this.fireEvent){
}
}
catch(err){
...
if(_3a && (!this.oninvalid || typeof this.oninvalid != 'function'))
this.oninvalid = new Function('event', _3a);
try {
if(this.oninvalid){
...
getOriginalAttrNode: function (node, _3) {
var r;
var dataSetItemName = _3 + 'AttrNode';
if ($wf2.getDatasetItem(node, dataSetItemName) == null) {
r = $wf2.copyOf(node.getAttributeNode(_3));
...
_6a : function(node){
var _10, _11, _29;
_10 = $wf2.getOriginalAttrNode(node, 'min');
_11 = $wf2.getOriginalAttrNode(node, 'max');
_29 = $wf2.getOriginalAttrNode(node, 'value');
node.min = undefined;
node.max = undefined;
node.step = undefined;
...
_76.multiline = false;
if(_76)
node.validity.patternMismatch = !_76.test(node.value);
}
if(_2e || _15)
node.validity.typeMismatch = ((node.wf2Value = $wf2._56(node.value, type)) == null);
else {
switch(type){
case 'number':
case 'range':
...
if(_10 && $wf2._63.test(_10.value))
node.min = Number(_10.value);
if(_11 && $wf2._63.test(_11.value))
node.max = Number(_11.value);
}
...
if(_10 && /^\d+$/.test(_10.value))
node.min = Number(_10.value);
else node.min = 0;
if(_11 && /^\d+$/.test(_11.value))
node.max = Number(_11.value);
else node.max = 1;
}
else {
...
var stepAttrNode = $wf2.getOriginalAttrNode(node, 'step');
if(!stepAttrNode){
node.step = _15 ? 60 : 1;
}
else if(stepAttrNode.value == 'any')
node.step = 'any';
else if($wf2._63.test(stepAttrNode.value) && stepAttrNode.value > 0)
...
node.wf2StepDatum = $wf2._43[type] ? $wf2._43[type] : 0;
var _step = node.step;
...
_5d : function(node){
if(node.validity && node.validity.typeMismatch !== undefined && !$wf2.hasBadImplementation)
return node;
...
ol.appendChild($wf2._48($wf2.invalidMessages.valueMissing));
if(target.validity.typeMismatch)
ol.appendChild($wf2._48($wf2.invalidMessages.typeMismatch.replace(/%s/, type)));
if(target.validity.rangeUnderflow)
ol.appendChild($wf2._48($wf2.invalidMessages.rangeUnderflow.replace(/%s/, $wf2._6c(target.min, type))));
if(target.validity.rangeOverflow)
ol.appendChild($wf2._48($wf2.invalidMessages.rangeOverflow.replace(/%s/, $wf2._6c(target.max, type))));
if(target.validity.stepMismatch)
ol.appendChild($wf2._48($wf2.invalidMessages.stepMismatch.replace(/%s/, target.step + ($wf2.stepUnits[type] ? ' ' + $wf2.stepUnits[type] + '(s)' : '')).replace(/%s/, $wf2._6c(target.wf2StepDatum, type))));
if(target.validity.tooLong)
ol.appendChild($wf2._48($wf2.invalidMessages.tooLong.replace(/%s/, target.maxLength).replace(/%s/, target.wf2ValueLength ? target.wf2ValueLength : target.value.length)));
if(target.validity.patternMismatch)
ol.appendChild($wf2._48($wf2.invalidMessages.patternMismatch.replace(/%s/, target.title ? target.title : ' "' + target.getAttribute('pattern') + '"')));
if(target.validity.customError)
ol.appendChild($wf2._48(target.validationMessage));
if(ol.childNodes.length == 1)
ol.className = 'single';
msg.appendChild(ol);
var _2 = document.body ? document.body : document.documentElement;
if($wf2._44.length)
_2.insertBefore(msg, $wf2._44[$wf2._44.length-1].errorMsg);
else
_2.insertBefore(msg, null);
var el = target;
...
var _1f;
while(_1f = $wf2._44[0]){
if(_1f.errorMsg && _1f.errorMsg.parentNode)
_1f.errorMsg.parentNode.removeChild(_1f.errorMsg);
...
var _e = [];
if(node.name)
_e.name = _1 ? _1(node.name) : node.name;
if(node.type == 'radio')
_e.type = node.type;
if(node.checked)
_e.checked = 'checked';
...
if((_4.specified || $wf2._4a[_4.name])
&& !$wf2._4b[_4.name] && (
(!_37 || (_12 > 1 || !$wf2._4c[_4.name]))
))
{
...
clone.htmlFor = (_1 ? _1(node.htmlFor) : node.htmlFor);
...
getAttributeByName: function (obj, _3) {
var i;
var attributes = obj.attributes;
for (i=0; i<attributes.length; i++) {
var _4 = attributes[i]
if (_4.nodeName == _3 && _4.specified) {
return _4;
}
}
return null;
},
getAttributeValue: function (obj, _3) {
var _4 = $wf2.getAttributeByName(obj, _3);
if (_4 != null) {
return _4.nodeValue;
} else {
return null;
...
setAttributeValue: function (obj, _3, _8) {
var _4 = $wf2.getAttributeByName(obj, _3);
if (_4 != null) {
_4.nodeValue = _8;
} else {
...
var _3 = 'data-' + name.toLowerCase();
var val = $wf2.setAttributeValue(obj, _3, value);
if ($wf2.getAttributeValue(obj, _3) == null) {
obj[_3] = value;
}
},
_50 : function(){
...
_53 : function(el, _74) {
if(el.currentStyle)
return el.currentStyle[_74];
else if(window.getComputedStyle)
return getComputedStyle(el, '').getPropertyValue(_74);
else if(el.style)
return el.style[_74];
else return '';
},
...
ms = '.' + $wf2._58(date.getUTCMilliseconds(), 3).replace(/0+$/,'');
switch(type){
case 'date':
return date.getUTCFullYear() + '-' + $wf2._58(date.getUTCMonth()+1) + '-' + $wf2._58(date.getUTCDate());
case 'datetime-local':
return date.getFullYear() + '-' + $wf2._58(date.getMonth()+1) + '-' + $wf2._58(date.getDate()) + 
'T' + $wf2._58(date.getHours()) + ':' + $wf2._58(date.getMinutes()) + ':' + $wf2._58(date.getMinutes()) + ms + 'Z';
case 'month':
...
default:
return date.getUTCFullYear() + '-' + $wf2._58(date.getUTCMonth()+1) + '-' + $wf2._58(date.getUTCDate()) + 
'T' + $wf2._58(date.getUTCHours()) + ':' + $wf2._58(date.getUTCMinutes()) + ':' + $wf2._58(date.getUTCMinutes()) + ms + 'Z';
}
},
_70 : function(_25){
...
if(_6._3 == 'autofocus'){
if(_6.attrChange == _6.ADDITION)
$wf2._5b(_6.target);
else if(_6.attrChange == _6.REMOVAL)
_6.target.autofocus = false;
}
}, false);
}
(function(){
...
return;
if(!_6.element && _6.relatedNode)
_6.element = _6.relatedNode;
if(!_6.element || !_6.element.repetitionTemplate)
return;
var rt = _6.element.repetitionTemplate;
var _3 = 'on' + _6.type;
var _2d = rt.getAttribute(_3) || rt.getAttribute(_1a[_6.type]);
if(_2d && (!rt[_3] || typeof rt[_3] != 'function'))
rt[_3] = new Function('event', _2d);
if(_6.element.repetitionTemplate[_3])
_6.element.repetitionTemplate[_3](_6);
else if(_6.element.repetitionTemplate[_1a[_6.type]])
_6.element.repetitionTemplate[_1a[_6.type]](_6);
}
